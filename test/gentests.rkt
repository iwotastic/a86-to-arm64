...
; Shim the run function
(define (run . exs) exs)

; Shim check-equal?
(define (check-equal? exs exp)
   (let* ((filebase (symbol->string (gensym 'test)))
          (rktfile (open-output-file (string-append filebase ".rkt")))
          (shellfile (open-output-file (string-append filebase ".sh")))
          (expfile (open-output-file (string-append filebase ".exp"))))
         (begin
            ; Generate loot code
            (display "#lang racket\n\n" rktfile)
            (map (lambda (ex) (begin (write ex rktfile) (display "\n" rktfile))) exs)

            ; Generate shell script
            (display "#!/bin/sh\n\n" shellfile)
            (display (string-append "cp test/" filebase ".rkt " filebase ".rkt\n") shellfile)
            (display (string-append "make -s -f arm.mk " filebase ".run\n") shellfile)
            (display (string-append "rm " filebase ".rkt\n") shellfile)
            (display (string-append "./" filebase ".run > " filebase ".out\n") shellfile)
            (display (string-append "echo '| running " filebase "'\n") shellfile)
            (display (string-append "diff " filebase ".out test/" filebase ".exp && echo '+-> PASS'\n") shellfile)
            (display (string-append "rm " filebase ".run " filebase ".out\n") shellfile)

            ; Generate expectation
            (if (not (or (char? exp) (string? exp) (number? exp) (boolean? exp) (equal? exp 'err))) (display "'" expfile) '())
            (write exp expfile)
            (display "\n" expfile)

            ; Clean up
            (close-output-port rktfile)
            (close-output-port shellfile)
            (close-output-port expfile))))
